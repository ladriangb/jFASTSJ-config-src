...
public class ActualizarVistas {
    ...
    public ActualizarVistas() {
        ...
        String listaDiagnostico = " DROP VIEW IF EXISTS view_listadiagnostico CASCADE;"
                ...
                + " ELSE (-1)"
                + " END AS id,"
                + " siniestro0_.id AS siniestro_id, detallesin1_.id AS detallesiniestro_id,"
                + " diag.diagnostico_id as diagnostico_id"
        ...
        String sumaPartida = " DROP VIEW IF EXISTS view_sumapartida CASCADE;"
                ...
                + " SELECT (((detallesin0_.ordendepago_id || '-') || tipocontra4_.partidapresupuestaria_id) || '-') || tipocontra4_.id AS id, detallesin0_.ordendepago_id, tipocontra4_.partidapresupuestaria_id, tipocontra4_.id AS tipocontrato_id, count(sumadetall0_.id) AS cantidaddetalles, round(sum("
                + " CASE"
                + " WHEN titular3_.persona_id = asegurado8_.persona_id THEN 1"
                + " ELSE 0"
                + " END), 2) AS numerosiniestrostitular, round(sum("
                + " CASE"
                + " WHEN titular3_.persona_id <> asegurado8_.persona_id THEN 1"
                + " ELSE 0"
                + " END), 2) AS numerosiniestrosfamiliar, round(sum("
                + " CASE"
                + " WHEN titular3_.persona_id = asegurado8_.persona_id THEN sumadetall0_.totalliquidado"
                + " ELSE 0"
                + " END), 2) AS montotitulares, round(sum("
                + " CASE"
                + " WHEN titular3_.persona_id <> asegurado8_.persona_id THEN sumadetall0_.totalliquidado"
                + " ELSE 0"
                + " END), 2) AS montofamiliares, round(sum(sumadetall0_.baseislr), 2) AS baseislr, round(sum(sumadetall0_.baseiva), 2) AS baseiva, round(sum(sumadetall0_.cantidadfacturas), 2) AS cantidadfacturas, round(sum(sumadetall0_.gastosclinicos), 2) AS gastosclinicos, round(sum(sumadetall0_.honorariosmedicos), 2) AS honorariosmedicos, round(sum(sumadetall0_.montoamparado), 2) AS montoamparado, round(sum(sumadetall0_.montoretenciondeducible), 2) AS montoretenciondeducible, round(sum(sumadetall0_.montoretencionprontopago), 2) AS montoretencionprontopago, round(sum(sumadetall0_.montoiva), 2) AS montoiva, round(sum(sumadetall0_.montonoamparado), 2) AS montonoamparado, round(sum(sumadetall0_.montoretencionislr), 2) AS montoretencionislr, round(sum(sumadetall0_.montoretencioniva), 2) AS montoretencioniva, round(sum(sumadetall0_.montoretenciontm), 2) AS montoretenciontm, round(sum(sumadetall0_.totalacancelar), 2) AS totalacancelar, round(sum(sumadetall0_.totalfacturado), 2) AS totalfacturado, round(sum(sumadetall0_.totalliquidado), 2) AS totalliquidado, round(sum(sumadetall0_.totalretenido), 2) AS totalretenido"
                + " FROM sini_detallesiniestro detallesin0_"
                + " CROSS JOIN view_sumadetalle sumadetall0_"
                ...
                + " CROSS JOIN aseg_asegurado asegurado8_"
                + " WHERE detallesin0_.ordendepago_id is not null AND"
                + " detallesin0_.siniestro_id = siniestro1_.id AND "
                + " siniestro1_.asegurado_id = asegurado8_.id AND "
                + " sumadetall0_.id = detallesin0_.id AND "
                + "siniestro1_.certificado_id = certificad2_.id AND"
                + " certificad2_.titular_id = titular3_.id AND"
                + " detallesin0_.tipocontrato_id = tipocontra4_.id"
                + " GROUP BY tipocontra4_.partidapresupuestaria_id, detallesin0_.ordendepago_id, tipocontra4_.id;"
        ...
        String sumaPartidaRemesa = " CREATE OR REPLACE VIEW view_sumapartidaremesa AS "
                + " SELECT pago_ordendepago.remesa_id || '-' || view_sumapartida.partidapresupuestaria_id || '-' || view_sumapartida.tipocontrato_id  AS id,"
                + " pago_ordendepago.remesa_id,"
                + " view_sumapartida.partidapresupuestaria_id, "
                + " view_sumapartida.tipocontrato_id, "
                + " sum(view_sumapartida.cantidaddetalles) AS cantidaddetalles,"
                + " sum(view_sumapartida.numerosiniestrostitular) AS numerosiniestrostitular, "
                + " sum(view_sumapartida.numerosiniestrosfamiliar) AS numerosiniestrosfamiliar,"
                + " sum(view_sumapartida.montotitulares) AS montotitulares, "
                + " sum(view_sumapartida.montofamiliares) AS montofamiliares,"
                + " sum(view_sumapartida.cantidadfacturas) AS cantidadfacturas,"
                + " sum(view_sumapartida.baseislr) AS baseislr,"
                + " sum(view_sumapartida.baseiva) AS baseiva, "
                + " sum(view_sumapartida.gastosclinicos) AS gastosclinicos,"
                + " sum(view_sumapartida.honorariosmedicos) AS honorariosmedicos, "
                + " sum(view_sumapartida.montoamparado) AS montoamparado,"
                + " sum(view_sumapartida.montoretenciondeducible) AS montoretenciondeducible, "
                + " sum(view_sumapartida.montoretencionprontopago) AS montoretencionprontopago,"
                + " sum(view_sumapartida.montoiva) AS montoiva, "
                + " sum(view_sumapartida.montonoamparado) AS montonoamparado, "
                + " sum(view_sumapartida.montoretencionislr) AS montoretencionislr, "
                + " sum(view_sumapartida.montoretencioniva) AS montoretencioniva,"
                + " sum(view_sumapartida.montoretenciontm) AS montoretenciontm,"
                + " sum(view_sumapartida.totalacancelar) AS totalacancelar,"
                + " sum(view_sumapartida.totalfacturado) AS totalfacturado,"
                + " sum(view_sumapartida.totalliquidado) AS totalliquidado,"
                + " sum(view_sumapartida.totalretenido) AS totalretenido"
                + " FROM view_sumapartida, pago_ordendepago"
                + " WHERE pago_ordendepago.id = view_sumapartida.ordendepago_id AND remesa_id is not null"
                + " GROUP BY pago_ordendepago.remesa_id, view_sumapartida.partidapresupuestaria_id,view_sumapartida.tipocontrato_id ;"
